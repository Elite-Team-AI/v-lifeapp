...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      log.error("Supabase credentials not configured", new Error("Missing Supabase credentials"), undefined, { userId, workoutId })
      return NextResponse.json(
        {
          error: 'Database service not configured',
          message: 'Unable to start workout session. Please contact support.',
